...
export function Api570SoilResistivityCalculator({ onBack, onNeedProject, notify, projects, initialCalculation, onSave, onReview, onApprove }: Api570CalculatorWorkflowProps) {
  ...
  const input = useMemo<Api570SoilResistivityInputSI>(() => ({ pinSpacingFt: pinSpacingMm / 304.8, resistanceOhm }), [pinSpacingMm, resistanceOhm]);
  ...
  const reportDefinition: Api570WorkflowReportDefinition = { reportKind: "Soil resistivity calculation report", basisTitle: "Four-pin field inputs", inspectionTitle: "Normalized equation inputs", summaryLines: [`Soil resistivity: ${formatDisplayNumber(result.soilResistivityOhmCm)} ohm-cm`], basisRows: [{ label: "Formula basis", value: "rho = 191.5 x d x R" }, { label: "Entered pin spacing", value: `${fields.pinSpacing.value} ${fields.pinSpacing.unit}` }, { label: "Entered resistance", value: `${fields.resistance.value} ${fields.resistance.unit}` }], inspectionRows: [{ label: "Pin spacing used", value: `${formatDisplayNumber(result.pinSpacingFtUsed)} ft` }, { label: "Resistance used", value: `${formatDisplayNumber(result.resistanceOhmUsed)} ohm` }], resultRows: [{ label: "Soil resistivity", value: `${formatDisplayNumber(result.soilResistivityOhmCm)} ohm-cm`, primary: true }] };
  ...
  const resultDisplay = formatDisplayNumber(result.soilResistivityOhmCm);
      ...
      <div className="calculator-workspace">
        <div className="input-column">
          <section className="form-card">
            ...
            <div className="form-grid">
              <label className="field"><span>Formula basis</span><div className="select-control">API 574 10.10.1.4.3 · ρ = 191.5 d R</div><small>Protected implementation expects d in feet and R in ohms; output is ohm-centimetres.</small></label>
              <label className="field"><span>Unit system</span><select aria-label="Unit system" className="select-control native-select" value={unitSystem} onChange={(event) => setUnitSystem(event.target.value as UnitSystem)}><option value="metric">Metric context · m / Ω</option><option value="us-customary">U.S. customary context · ft / Ω</option></select><small>The result remains Ω·cm, matching the protected calculator; this selection changes the visible normalized spacing context.</small></label>
            </div>
            <div className="form-note is-valid"><ShieldCheck size={17} /><p><strong>Final piping equation verified.</strong> Equivalent ft, m, in, cm, Ω, kΩ, and MΩ inputs reproduce the same result.</p></div>
          </section>
          ...
          <section className="form-card">
            ...
            <div className="form-grid">
              <UnitInput label="Pin spacing d" field={fields.pinSpacing} options={lengthUnits} help="Distance between equally spaced pins. Converted to feet for the protected equation." onValueChange={(value) => updateFieldValue("pinSpacing", value)} onUnitChange={(unit) => updateFieldUnit("pinSpacing", unit)} />
            ...
            <div className="unit-system-note"><Info size={17} /><p><strong>Live conversion is active.</strong> The engine receives <b>{formatDisplayNumber(result.pinSpacingFtUsed)} ft</b> and <b>{formatDisplayNumber(result.resistanceOhmUsed)} Ω</b>; normalized spacing context is <b>{spacingDisplay}</b>.</p></div>
          </section>
        ...
        <aside className="result-column">
          <section className="result-card">
            ...
            <div className="result-value"><strong>{resultDisplay}</strong><span>Ω·cm</span></div>
            <div className="result-comparison"><span>Pin spacing used<strong>{formatDisplayNumber(result.pinSpacingFtUsed)} ft</strong></span><span>Resistance used<strong>{formatDisplayNumber(result.resistanceOhmUsed)} Ω</strong></span></div>
            <div className={`result-status ${result.ok ? "is-valid" : ""}`}>{result.ok ? <CircleCheck size={18} /> : <TriangleAlert size={18} />}<div><strong>{error ? "Resolve input issues" : "Calculation completed"}</strong><span>{error?.message ?? "The protected 191.5 × d(ft) × R(Ω) dependency is active."}</span></div></div>
          </section>
          <section className="trace-card">
            ...
            <div><span>Formula</span><strong>ρ = 191.5 × d × R</strong></div>
            <div><span>Pin spacing d</span><strong>{formatDisplayNumber(result.pinSpacingFtUsed)} ft</strong></div>
            <div><span>Resistance R</span><strong>{formatDisplayNumber(result.resistanceOhmUsed)} Ω</strong></div>
            <div><span>Soil resistivity ρ</span><strong>{formatDisplayNumber(result.soilResistivityOhmCm)} Ω·cm</strong></div>
          </section>
        </aside>
      </div>
    </div>
