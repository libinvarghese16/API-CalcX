...
function makeNozzle(index: number): NozzleState {
  ...
  return {
    ...
    fields: {
      ...
      pressureMinimumThickness: { value: golden ? "2" : "", unit: "mm", quantity: "length" },
...
export function Api653NozzleCalculator({ onBack }: { onBack: () => void }) {
  ...
  const input = useMemo<Api653NozzleAssessmentInputSI>(() => ({
    ...
    nozzles: nozzles.map((nozzle, index) => ({
      ...
      pressureMinimumThicknessMm: unitValue(nozzle.fields.pressureMinimumThickness),
        ...
        <div className="shell-course-list">{nozzles.map((nozzle, index) => {
          ...
          return <article className={`shell-course-card ${nozzle.minimumThicknessMode === "manual" ? "has-manual" : ""}`} key={`nozzle-${index + 1}`}><div className="shell-course-header"><div><span>{String(index + 1).padStart(2, "0")}</span><div><p>Nozzle {index + 1}</p><small>{nozzle.detail || "Inspection entry available"}</small></div></div><div className="shell-course-header-result"><span>Remaining life</span><strong>{inactive ? "—" : lifeDisplay(nozzleResult.remainingLifeYears)}{!inactive && nozzleResult.remainingLifeYears !== null ? " yr" : ""}</strong></div></div>
            <div className="shell-course-inputs"><label className="field"><span>Nozzle detail<button type="button" title="Equipment drawing designation or service description." aria-label={`Nozzle ${index + 1} detail help`}>?</button></span><input className="select-control shell-select-input" aria-label={`Nozzle ${index + 1} detail`} type="text" value={nozzle.detail} placeholder={`N${index + 1} service`} onChange={(event) => updateNozzle(index, (entry) => ({ ...entry, detail: event.target.value }))} /><small>Original nozzle identification or service text.</small></label>
              ...
              <UnitInput label={`Pressure-design minimum · N${index + 1}`} field={nozzle.fields.pressureMinimumThickness} options={lengthUnits} help="Required from the controlled pressure-design nozzle calculation; it is assessed separately from the structural table minimum." onValueChange={(value) => updateFieldValue(index, "pressureMinimumThickness", value)} onUnitChange={(unit) => updateFieldUnit(index, "pressureMinimumThickness", unit)} />
            ...
            <div className="shell-course-results"><span className={!inactive && nozzleResult.corrosionAllowanceMm <= 0 ? "is-alert" : ""}><small>Corrosion allowance</small><strong>{inactive ? "—" : `${formatThickness(nozzleResult.corrosionAllowanceMm)} ${thicknessUnit}`}</strong>{!inactive && nozzleResult.corrosionAllowanceMm <= 0 && <em>Minimum reached</em>}</span><span><small>Structural Tmin</small><strong>{inactive ? "—" : `${formatThickness(nozzleResult.structuralMinimumThicknessMmUsed)} ${thicknessUnit}`}</strong></span><span><small>Pressure Tmin</small><strong>{inactive ? "—" : `${formatThickness(nozzleResult.pressureMinimumThicknessMmUsed)} ${thicknessUnit}`}</strong></span><span><small>Governing Tmin</small><strong>{inactive ? "—" : `${formatThickness(nozzleResult.minimumThicknessMmUsed)} ${thicknessUnit}`}</strong></span></div>
            <div className="shell-course-trace"><p><b>Structural Tmin:</b> {nozzle.minimumThicknessMode === "manual" ? "highlighted manual override" : nozzleResult.minimumSelection.message}</p><p><b>Governing basis:</b> {nozzleResult.governingMinimumBasis}. <b>RL:</b> (t actual − max(Tmin structural, Tmin pressure)) ÷ max(CR long, CR short).</p></div>
          </article>;
        })}</div>
      </section>
