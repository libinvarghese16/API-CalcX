...
export function Api653PlateRemainingLifeCalculator({ onBack, variant }: { onBack: () => void; variant: Api653PlateVariant }) {
  ...
  const copy = isAnnular ? {
    ...
    routeMessage: "The dependency chain calculates shell stress, selects annular Tmin, derives RTip from the current pit depth, and evaluates bottom- and top-side annular metal loss.",
      ...
      <section className="form-card">
        ...
        <div className="form-grid">
          ...
          {!isAnnular && <>
            ...
            <AutomaticUnitInput label="Top-side corrosion rate StPr" field={topSideRateMode === "auto" ? { ...fields.topSideRate, value: formatInput(convertSIToUnit(bottomResult.automaticTopSideCorrosionRateMmPerYear, "length", fields.topSideRate.unit)) } : fields.topSideRate} options={lengthUnits} mode={topSideRateMode} help="Automatic StPr is the larger long- or short-term pit-depth rate. The short-term route uses the current depth over the years since the previous inspection." onValueChange={(value) => updateFieldValue("topSideRate", value)} onUnitChange={(unit) => updateFieldUnit("topSideRate", unit)} onModeChange={setTopSideRateMode} />
          </>}
        </div>
        <div className={`form-note ${manualOverrideActive ? "is-manual" : "is-valid"}`}>{manualOverrideActive ? <TriangleAlert size={18} /> : <Info size={17} />}<p><strong>{manualOverrideActive ? "Manual override active." : "Automatic dependencies active."}</strong> {manualOverrideActive ? "Verify every highlighted manual field before engineering review." : isAnnular ? "Annular RTip and pitting rates are derived automatically from the current pit depth." : "MRT uses separate RTbc, derived RTip, UPr, StPr, and projection interval inputs."}</p></div>
