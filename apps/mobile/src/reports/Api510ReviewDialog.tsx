...
export function Api510ReviewDialog({ model, confirmed, onConfirmedChange, onRecordReview, onClose, onContinue }: {
  ...
  const reviewItems = [
    { icon: Gauge, label: "Calculation validation", value: !model.resultOk ? "Input errors remain" : hasWarnings ? "Arithmetic complete; scope review required" : "Calculation completed", ok: model.resultOk && !hasWarnings },
    { icon: ShieldCheck, label: "Input traceability", value: "Calculation basis recorded", ok: true },
  ...
  return (
    <div className="modal-backdrop review-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}>
      <section className="modal-card calculation-review-modal" role="dialog" aria-modal="true" aria-labelledby="calculation-review-title">
        ...
        <section className="review-section"><div className="review-section-heading"><div><p className="eyebrow">Calculation output</p><h3>Governing result summary</h3></div><span>{model.resultOk ? "Calculation complete" : "Review required"}</span></div><ReviewRows rows={model.resultRows} /></section>
        {model.issues.length ? <section className="review-issue-panel"><TriangleAlert size={18} /><div><strong>Calculation issues</strong>{model.issues.map((issue, index) => <p key={`${issue.message}-${index}`}>{issue.severity.toUpperCase()} · {issue.message}</p>)}</div></section> : <section className="review-issue-panel is-clear"><CircleCheck size={18} /><div><strong>No calculation issues</strong><p>The result contains no calculation errors or warnings.</p></div></section>}
