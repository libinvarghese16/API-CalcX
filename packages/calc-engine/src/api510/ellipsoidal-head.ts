...
function requirePositive(
  ...
  field: "insideDiameterMm" | "designPressureMpa" | "allowableStressMpa" | "jointEfficiency",
  value: number,
  label: string,
): void {
...
/** Pure extraction of the protected legacy API 510 ellipsoidal-head path. */
export function calculateEllipsoidalHead(input: CylindricalShellInputSI): EllipsoidalHeadResultSI {
  const issues: CalculationIssue[] = [];
  requirePositive(issues, "insideDiameterMm", input.insideDiameterMm, "Inside diameter");
  ...
  if (pressureBasisValid) {
    ...
    if (denominator > 0) requiredThicknessMm = (input.designPressureMpa * input.insideDiameterMm) / denominator;
    else issues.push({ code: "ellipsoidal-denominator-not-positive", field: "calculation", severity: "error", message: "The ellipsoidal-head required-thickness denominator must be greater than zero." });
  }
  ...
  const mawpFromThickness = (thicknessMm: number): number => {
    if (!(pressureBasisValid && thicknessMm > 0 && input.insideDiameterMm > 0)) return 0;
    const denominator = input.insideDiameterMm + (0.2 * thicknessMm);
